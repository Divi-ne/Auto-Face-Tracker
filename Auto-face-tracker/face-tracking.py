...
arduino = serial.Serial('COM9', 9600, timeout=1)  # Change COM9 to your Arduino port
...
while True:
    ...
    if len(faces) > 0:
        ...
        if abs(diff) < FRAME_CENTER_TOLERANCE:
            ...
            command = 'R'  # Face is to the right → turn motor right
        else:
            command = 'L'  # Face is to the left → turn motor left
